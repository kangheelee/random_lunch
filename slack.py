...
# 채널 이름에 맞게 수정 필요
LUNCH_CHANNEL_NAME = "team_zep_lunch"

ssl._create_default_https_context = ssl._create_unverified_context

# Github의 경우 setting -> settings -> Secrets에서 환경 변수 설정
# 로컬 환경 터미널의 경우 export SLACK_BOT_TOKEN="xoxb-?????"
...
def get_user_ids(channel_name=LUNCH_CHANNEL_NAME):
...
def get_offstage_channel_id(channel_name=LUNCH_CHANNEL_NAME):
    try:
        # search for channels first
        # type의 디폴트는 public 채널 조회라서, private_channel이라는 인수를 넣어준다.
        result = client.conversations_list(
            exclude_archived=True, limit=1000, types='private_channel')
